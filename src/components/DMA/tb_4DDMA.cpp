...
DMA_4D dma_4d_mm2s("DMA_4D_MM2S", DmaDirection::MM2S, clk, reset, enable, ram, stream);
DMA_4D dma_4d_s2mm("DMA_4D_S2MM", DmaDirection::S2MM, clk, reset, enable, ram, stream);
...
bool run2DStridedTransferTest()
{
	...
	Descriptor_4D dma_4d_mm2s_transfer = {1, 0, DmaState::TRANSFER, 5, 2, 4, 12, 0, 0, 0, 0}; 	
	Descriptor_4D dma_4d_mm2s_suspend = {1, 0, DmaState::SUSPENDED, 10, 1, 0, 0, 0, 0, 0, 0}; 	
	Descriptor_4D dma_4d_s2mm_wait = {1, 0, DmaState::WAIT, 1, 1, 0, 0, 0, 0, 0, 0};			
	Descriptor_4D dma_4d_s2mm_transfer = {2, 100, DmaState::TRANSFER, 5, 2, 4, 12, 0, 0, 0, 0}; 
	Descriptor_4D dma_4d_s2mm_suspend = {2, 0, DmaState::SUSPENDED, 10, 1, 0, 0, 0, 0, 0, 0};	

	dma_4d_mm2s.loadProgram({dma_4d_mm2s_transfer,
							 dma_4d_mm2s_suspend});

	dma_4d_s2mm.loadProgram({dma_4d_s2mm_wait,
							 dma_4d_s2mm_transfer,
							 dma_4d_s2mm_suspend});
